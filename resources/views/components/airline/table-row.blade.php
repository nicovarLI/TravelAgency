...
<tr class="hover:bg-gray-300" data-id="{{$airline->id}}">
        ...
        <td>
            <button @click=" show = true; airlineName = '{{$airline->name}}'; airlineId = '{{$airline->id}}'; airlineDescription = '{{$airline->description}}';" onclick="loadCitySelect({{$airline->id}})" class="text-xs bg-blue-400 text-white hover:bg-white action:bg-red-500r hover:text-blue-500 p-2 px-4 rounded-full">
                Edit
            </button>
        </td>
        <td>
